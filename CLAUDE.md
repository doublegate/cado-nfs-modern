...
- **Versioning:** `CADO_VERSION_STRING` is `3.3.0-modern` (`CMakeLists.txt`: `CADO_VERSION_MINOR 3` + `-modern` suffix). The numeric minor **deliberately diverges** from upstream 3.0.0 to mark the fork's substantial original work; no test depends on the string.
...
- `docs/gpu-lingen-ntt-c6.md` — (3.3.0, C6) GPU GF(p) lingen NTT (bit-exact vs schoolbook); the single-prime inner transform of a multi-modular GF(p) lingen; <1% single-machine net, multi-GPU/DLP play.
- `BENCHMARKS.md` — performance sweep + methodology (§7 = the 3.3.0 additions).
